...
export default function DigitalPassCard({ personName, vendorName, marketName, validRange, photo }) {
  ...
  return (
    <div style={{ position:'relative', width:'100%', maxWidth:340, margin:'0 auto' }}>
      <div style={{ position:'absolute', top:-46, left:-46, width:160, height:160, borderRadius:'50%', background:'radial-gradient(circle at 40% 40%, #E8A05C, #B97434 60%, transparent 75%)', filter:'blur(26px)', opacity:0.45, pointerEvents:'none' }}/>
      <div onClick={() => setOpen(o => !o)} style={{ position:'relative', cursor:'pointer', paddingTop: open ? 226 : 0, transition:'padding-top 0.7s cubic-bezier(0.45,0,0.2,1)' }}>
        <div style={{ position:'relative', width:'100%', height:226, perspective:1400 }}>
          ...
          <div style={{
            ...
            boxShadow:'0 18px 44px rgba(42,23,8,0.35)',
            ...
            <div style={{ position:'relative', padding:'15px 18px 16px', display:'flex', flexDirection:'column', height:'100%', boxSizing:'border-box' }}>
              <div style={{ fontFamily:"'Marcellus',serif", fontSize:19, letterSpacing:'0.05em', color:'#FFF3E2', textTransform:'uppercase', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{personName || 'Unnamed pass holder'}</div>
          ...
          <div style={{ position:'absolute', inset:0, transformOrigin:'top center', transformStyle:'preserve-3d', transition:'transform 0.7s cubic-bezier(0.45,0,0.2,1)', transform: open ? 'rotateX(-180deg)' : 'rotateX(0deg)', zIndex:2 }}>
            ...
            <div style={{ position:'absolute', inset:0, backfaceVisibility:'hidden', WebkitBackfaceVisibility:'hidden', borderRadius:20, overflow:'hidden', background:'rgba(255,248,238,0.13)', backdropFilter:'blur(18px)', WebkitBackdropFilter:'blur(18px)', border:'1px solid rgba(255,248,238,0.32)', boxShadow:'0 18px 44px rgba(0,0,0,0.4)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', textAlign:'center', padding:'18px 20px', boxSizing:'border-box' }}>
